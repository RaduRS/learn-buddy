...
export async function POST(request: NextRequest) {
  try {
    const { userAge, difficulty = 1, theme } = await request.json() as PuzzleRequest

    const nebiusApiKey = process.env.NEBIUS_API_KEY
    if (!nebiusApiKey) {
      return NextResponse.json({ error: 'Nebius API key not configured' }, { status: 500 })
    }
    ...
    const chosenTheme = theme || defaultThemes[Math.floor(Math.random() * defaultThemes.length)]
